      ...
      <v-card-text>
        <v-form ref="form" v-model="valid">
          <div>
            <v-row>
              <v-col cols="6">
                <v-text-field v-model="factoryData.id" label="ID" disabled></v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="5">
                <v-text-field
                  v-model="factoryData.shortName"
            ...
            <v-row>
              <v-col cols="6">
                <v-autocomplete
                  v-model="factoryData.buildingIds"
                  :items="buildings"
...
export default {
  ...
  methods: {
    ...
    saveItem() {
      this.$emit('save', { ...this.factoryData });
      this.resetForm();
    },
  },
  watch: {
    ...
    showForm: {
      ...
      handler(show) {
        if (show && !this.editingData) {
          this.factoryData.id = this.generateID();
        }
      },
    },
  },
