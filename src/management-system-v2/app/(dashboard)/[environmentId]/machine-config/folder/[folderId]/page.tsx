...
export type ListItem = MachineConfig | (Folder & { type: 'folder' });

const MachineConfigsPage = async ({
  params,
}: {
  ...
  const folderContents = await asyncMap(getFolderChildren(folder.id, ability), async (item) => {
    if (item.type === 'folder') {
      ...
      return getMachineConfigById(item.id, ability); //TODO
    }
  }); //satisfies ListItem[]; //TODO
