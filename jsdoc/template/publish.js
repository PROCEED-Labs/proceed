...
const modClassesList = []; // my changes to see if class link already under module
...
function buildItemTypeStrings(item) {
  const types = [];
  const jsTypes = ['string', 'object', 'array', 'boolean', 'function', 'null', 'promise', 'number'];
  ...
  if (item && item.type && item.type.names) {
    item.type.names.forEach((name) => {
      // my changes
      if (jsTypes.includes(name.toLowerCase())) {
        const mdnLink = `<a href="${mdn}${name}">${name}</a>`;
        types.push(mdnLink);
      } else {
        // end my changes
...
function addMemberList(itemsNav, members) {
  itemsNav += "<ul class='modClassLinks'>";
  members.forEach((member) => {
    if (modClassesList.includes(member.longname)) {
      // don't add class link to nav again
      return;
    }
    itemsNav += '<li>';
    itemsNav += linkto(member.longname, member.name);
    itemsNav += '</li>';
    modClassesList.push(member.longname);
    const modmembers = find({ kind: 'module', memberof: member.longname });
    let memberMembers = modmembers;
    if (env.opts.includeClassInNav) {
      const classmembers = find({ kind: 'class', memberof: member.longname });
      memberMembers = [...modmembers, ...classmembers];
    }
    if (memberMembers.length) {
      itemsNav = addMemberList(itemsNav, memberMembers);
    }
  });
  itemsNav += '</ul>';
  return itemsNav;
  ...
  if (items.length) {
    ...
    items.forEach((item) => {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        // start my changes
        if (modClassesList.includes(item.longname)) {
          // don't add class link to nav again
          return;
        }
        if (!env.opts.includeClassInNav && item.kind === 'class') {
          return;
        }
        // end my changes
      ...
      // start my changes
      // add member of specified kind below modules
      const addMembersFor = ['Modules', 'Classes', 'Interfaces', 'Events'];
      if (addMembersFor.includes(itemHeading)) {
        const modmembers = find({ kind: 'module', memberof: item.longname });
        let members = modmembers;
        if (env.opts.includeClassInNav) {
          const classmembers = find({ kind: 'class', memberof: item.longname });
          members = [...modmembers, ...classmembers];
        }
        if (members.length) {
          itemsNav = addMemberList(itemsNav, members);
        }
      }
      // end my changes
    });
    ...
    if (itemsNav !== '') {
      nav += `<ul>${itemsNav}</ul>`; // `<h3>${itemHeading}</h3><ul>${itemsNav}</ul>`;
    }
  }
...
function buildGroupNav(members, title) {
  ...
  nav += `<div class="category" id="${title}">`;
  if (title) {
    nav += `<h3>${title}</h3>`;
  }
  ...
  if (members.globals && members.globals.length) {
    ...
    members.globals.forEach((g) => {
      // start my changes
      if (modClassesList.includes(g.longname)) {
        // don't add class link to nav again
        return;
      }
      // end my changes
    ...
    if (!globalNav) {
      ...
      nav += `<h5>${linkto('global', 'Global')}</h3>`;
    } else {
      nav += `<h5>Global</h3><ul>${globalNav}</ul>`;
    }
  }
  nav += '</div>';
  return nav;
...
function buildNav(members) {
  ...
  const categorised = {};
  const rootScope = {};
  const { noCategoryTitle } = env.opts;
  const types = [
    'tutorials',
    'modules',
    'externals',
  ...
  Object.keys(categorised).forEach((category) => {
    nav += buildGroupNav(categorised[category], category);
  });
  if (Object.keys(rootScope).length > 0) {
    nav += buildGroupNav(rootScope, noCategoryTitle);
  }
  return nav;
}
...
exports.publish = function (taffyData, opts, tutorials) {
  ...
  templatePath = path.getResourcePath('templates/default'); // my changes so everything but publish.js will be taken from jsdoc default path
  ...
  // const jsTypes = ['string', 'object', 'array', 'boolean', 'function', 'null', 'promise'];
  // const mdn = 'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/';

  data().each((doclet) => {
    let sourcePath;
    /* my changes -> param mdn link but link name = href
    if (doclet.params) {
      doclet.params.forEach((param) => {
        if (param.type && param.type.names) {
          param.type.names.forEach((name) => {
            if (jsTypes.includes(name.toLowerCase())) {
              const mdnLink = mdn + name;
              param.type.names.splice(param.type.names.indexOf(name), 1, mdnLink);
            }
          });
        }
      });
