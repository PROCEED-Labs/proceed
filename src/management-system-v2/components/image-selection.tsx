...
const ImageSelectionSection: React.FC<ImageSelectionSectionProperties> = ({ metaData }) => {
  ...
  const { mutateAsync: updateImage } = usePutAsset(
  ...
  const imageFileName = useMemo(() => {
    if (metaData.overviewImage) {
      const imageFileName = metaData.overviewImage.split('/images/').pop();
