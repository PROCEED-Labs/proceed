...
// TODO: there are plugins for pdfmake that convert markdown into its internal format but most of them are missing support for some of the elements available in our editor (e.g. Code-Blocks, Tables etc.)
// Maybe convert the markdown to html with something like this (https://www.npmjs.com/package/marked) and use this (https://www.npmjs.com/package/html-to-pdfmake) to then convert to the final pdfmake format

// TODO: there is a bug when using "auto" as a pagesize where it does not work anymore on pages after the first one (https://github.com/bpampuch/pdfmake/issues/2551)
// The workaround mentioned in the issue would be to create each page as a separate file and then merge the files using pdf-lib (https://www.npmjs.com/package/pdf-lib)
...
async function pdfExport(
  ...
  var docDefinition = {
    ...
    // Problems: "heigth: 'auto'" seems to clash with landscape orientation (results in an error) and "width: 'auto'" seemingly not possible
    pageSize: { width: 580, height: 'auto' },
  };

  const pdf = pdfMake.createPdf(docDefinition as any);

  const blob = (await new Promise((resolve) => pdf.getBlob((res) => resolve(res)))) as any;

  if (zip) {
    zip.file(`${processData.definitionName}.pdf`, blob);
  } else {
    downloadFile(`${processData.definitionName}.pdf`, blob);
  }
