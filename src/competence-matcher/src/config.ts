...
export const config = {
  ...
  splittingLength: parseInt(process.env.SPLITTING_LENGTH || '1000', 0), // Set this to 0 to disable splitting
  reasonModel: process.env.REASON_MODEL || 'llama3.2',
  splittingSymbol: process.env.SPLITTING_SYMBOL || '<SPLITTING_SYMBOL>',
  workerHeartbeatInterval: parseInt(process.env.WORKER_HEARTBEAT_INTERVAL || '120', 10) * 1_000,
  workerDeathTimeout: parseInt(process.env.WORKER_DEATH_TIMEOUT || '360', 10) * 1_000,
